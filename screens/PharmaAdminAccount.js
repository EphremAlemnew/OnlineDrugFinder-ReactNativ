...
React.useEffect(()=>{fetchAccount()},[])
  ...
  //  await axios
  //     .post(`http://${Api}/drugs/userList/`, userData)
  //     .then((response)=>{
  //       const userTableId=response.data.id;
  //       axios.post(`http://${Api}/drugs/accountList/`, {
  //         userName: userName,
    ...
    <SafeAreaView>
      ...
      {isVisible && (
        ...
        <Text style={{ color: 'red' }}>Please enter a valid email address</Text>
          ...
          onChangeText={setPhoneNo} placeholder="Phone number" 
          placeholderTextColor={Colors.darkText} 
      ...
      <TouchableOpacity onPress={()=>navigate('AdminPharma')} style={{alignItems:'center', backgroundColor:Colors.onPrimary,marginLeft:Layout.width*0/100,width:Layout.width*20/100,padding:Spacing}}>
            ...
            <Image style={{tintColor:Colors.primary,width:Layout.width*9/100,height:Layout.width*9/100}} source={require("../assets/images/inventory.png")}></Image>
               <Text style={{color:Colors.primary}}>Inventory</Text> 
            </TouchableOpacity>
